...
| Tên Phòng làm việc        | Mục đích & Lý do thiết kế                                                                                                      | Items được kéo vào (Dashboards, Filters, Spaces)                                                                               |
| :------------------------ | :----------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------- |
| **HR Team Workspace**     | Không gian làm việc tập trung cho toàn bộ đội ngũ Nhân sự để thực hiện các nghiệp vụ hàng ngày.                                | - **Spaces**: Hồ sơ Nhân sự, Tuyển dụng, Hội nhập & Thôi việc, Yêu cầu Nhân sự\<br\>- **Dashboard**: Báo cáo Tổng quan Nhân sự |
| **Manager's Workspace**   | Cung cấp cho các Trưởng phòng/Quản lý một giao diện để phê duyệt các yêu cầu từ nhân viên và xem thông tin cơ bản của đội ngũ. | - **Space**: Yêu cầu Nhân sự\<br\>- **Filter**: "Các yêu cầu đang chờ duyệt từ team của tôi"                                   |
| **Employee Self-Service** | Một cổng thông tin chung cho toàn bộ nhân viên, nơi họ có thể gửi yêu cầu và theo dõi trạng thái.                              | - **Space**: Yêu cầu Nhân sự\<br\>- **Filter**: "Các yêu cầu của tôi"                                                          |

## **PHẦN 2: QUẢN LÝ NGƯỜI DÙNG**

Phần này định nghĩa các tài khoản người dùng, nhóm, và các vai trò trừu tượng cần thiết để quản lý phân quyền và luồng công việc trong toàn bộ giải pháp HRM.

#### **2.1. Quản lý Tài khoản**

[cite_start]Hệ thống sẽ sử dụng hai loại tài khoản để phân biệt giữa các hành động do nhân viên thực hiện và các hành động do quy trình tự động hóa thực thi. [cite: 3]

| Loại Tài khoản           | Mô tả                                                                                                                                                                                                 | Áp dụng tại ACME Corp                                                                                                        |
| :----------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------- |
| **Tài khoản Người dùng** | [cite_start]Tài khoản tiêu chuẩn cho các thành viên, dùng để đăng nhập và làm việc tương tác trên nền tảng. [cite: 3]                                                                                 | Dành cho tất cả nhân viên của ACME Corporation.                                                                              |
| **Tài khoản Chức năng**  | [cite_start]Một tài khoản đặc biệt, không dùng để đăng nhập, được sử dụng riêng cho công cụ tự động hóa để thực thi các quy tắc, đảm bảo lịch sử hoạt động rõ ràng và phân quyền nhất quán. [cite: 3] | Một tài khoản duy nhất, `hr.automation@acme.com`, sẽ được tạo để thực thi tất cả các quy tắc tự động hóa HRM liên chức năng. |

#### **2.2. Nhóm Toàn cục (Global Groups)**

[cite_start]Nhóm là tập hợp các tài khoản người dùng, giúp cho việc gán quyền và thông báo trở nên đơn giản hơn. [cite: 3]
...
[cite_start]Vai trò là các chức danh trừu tượng được sử dụng trong các phương án phân quyền. [cite: 134] [cite_start]Người dùng hoặc nhóm cụ thể sẽ được gán vào các vai trò này trong từng Mảng việc. [cite: 134]
...
Phần này vạch ra các mối quan hệ giữa các loại công việc (`Object`) khác nhau napmgf trong các chức năng Nhân sự của bạn. Nền tảng Luklak cung cấp bốn phương pháp để tạo ra các kết nối này, mỗi phương pháp phục vụ một mục đích kiến trúc riêng biệt. Chúng ta sẽ sử dụng kết hợp các phương pháp này để xây dựng một mô hình vững chắc.

#### **3.1. Quan hệ Cha-con**

- **Mô tả:** Phương pháp này tạo ra một liên kết phân cấp chặt chẽ, trong đó một đối tượng "con" là một phần của đối tượng "cha". [cite_start]Đây là lựa chọn lý tưởng để chia nhỏ một hạng mục công việc lớn thành các nhiệm vụ nhỏ hơn, dễ quản lý. [cite: 65, 216]
- **Áp dụng tại ACME Corp:**

| Object Cha            | Object Con           | Lý do Kiến trúc                                                                                                                                                                                                                                 |
| :-------------------- | :------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Quy trình Hội nhập`  | `Nhiệm vụ Hội nhập`  | **Phân rã Công việc:** Một `Quy trình Hội nhập` là một quy trình phức tạp. Việc chia nhỏ nó thành các `Nhiệm vụ Hội nhập` riêng lẻ (VD: "Chuẩn bị Laptop," "Thiết lập Lương") cho phép giao việc và theo dõi các hạng mục con một cách rõ ràng. |
| `Quy trình Thôi việc` | `Nhiệm vụ Thôi việc` | **Phân rã Công việc:** Tương tự quy trình hội nhập, quy trình thôi việc được chia thành các nhiệm vụ riêng biệt (VD: "Bàn giao Tài sản," "Thu hồi Quyền truy cập") để đảm bảo trách nhiệm.                                                      |

#### **3.2. Kết nối Toàn cục**

- **Mô tả:** Đây là các loại mối quan hệ hai chiều, có thể tái sử dụng (VD: "liên quan đến," "gây ra bởi") được quản lý ở cấp toàn cục. [cite_start]Chúng giúp chuẩn hóa _ý nghĩa_ của sự kết nối giữa hai đối tượng bất kỳ trong hệ thống. [cite: 64, 215]
- **Áp dụng tại ACME Corp:**
...
#### **3.3. Trường Chọn Đối tượng**

- [cite_start]**Mô tả:** Loại trường này là cách thức triển khai thực tế của một mối quan hệ, cho phép người dùng chọn một hoặc nhiều đối tượng từ một chức năng khác và liên kết chúng với đối tượng hiện tại. [cite: 64, 215]
- **Áp dụng tại ACME Corp:**
...
#### **3.4. Trường Bảng dữ liệu**

- **Mô tả:** Một loại trường đặc biệt hoạt động như một 'bảng tính' được nhúng bên trong một đối tượng. [cite_start]Nó hoàn hảo để quản lý các danh sách dữ liệu có cấu trúc như dòng đơn hàng, checklist, hoặc nhật ký. [cite: 432]
- **Áp dụng tại ACME Corp:**
...
#### **4.1. Giao diện làm việc Toàn cục**
...
#### **5.2. Tự động hóa Liên Chức năng**
...
| 4 | CF-02 | **Bắt đầu Hội nhập → Tạo việc cho IT & Kế toán**| Việc thay đổi trạng thái trên `Hội nhập & Thôi việc/🧊Quy trình Hội nhập` sẽ tự động tạo các object con `Nhiệm vụ Hội nhập` và gán chúng cho phòng ban IT và Kế toán. |
...
| 7 | CF-05 | **Bắt đầu Thôi việc → Tạo việc cho IT & Kế toán**| Việc tạo ra một `Hội nhập & Thôi việc/🧊Quy trình Thôi việc` sẽ tự động tạo các object con `Nhiệm vụ Thôi việc` và gán chúng cho phòng ban IT và Kế toán để thực hiện. |

---
## **PHẦN 6: MÔ HÌNH PHÂN QUYỀN**

Phần này định nghĩa chiến lược bảo mật và kiểm soát truy cập cho toàn bộ giải pháp HRM, sử dụng mô hình bảo mật 3 lớp của Luklak.

#### **6.1. Lớp 1: Đặc quyền Doanh nghiệp**
...
#### **6.2. Lớp 2: Nguyên tắc Phân quyền Truy cập Tiện ích**
...
| **📋 Chức năng** | Quyền chỉnh sửa bốn Chức năng HRM được giới hạn cho người dùng có đặc quyền **App Manager**. | Bảo vệ tính toàn vẹn của các quy trình nghiệp vụ cốt lõi, đảm bảo chỉ các chuyên gia được ủy quyền mới có thể sửa đổi logic nền tảng của hệ thống HRM. |
...
#### **6.3. Lớp 3: Phương án Phân quyền & Thông báo**
